...
def determine_final_score(confidences):
    """
    Determine the final class based on the highest average confidence.

    Parameters:
    confidences (dict): A dictionary with class labels as keys and lists of confidence scores as values.
                        Example: {'a': [confidence_a], 'b': [confidence_b1, confidence_b2]}

    Returns:
    str: The final class with the highest average confidence.
    """

    # Calculate average confidence for each class
    average_confidences = {class_label: sum(scores) / len(scores) for class_label, scores in confidences.items()}

    # Determine the final class based on the highest average confidence
    final_decision = max(average_confidences, key=average_confidences.get)
...
input_img_clock = st.file_uploader("Enter your Image 1", type=['jpeg', 'jpg', 'png'])
input_img_spiral = st.file_uploader("Enter your Image 2", type=['jpeg', 'jpg', 'png'])
input_img_wave = st.file_uploader("Enter your Image 3", type=['jpeg', 'jpg', 'png'])
...
if input_img_clock is not None and input_img_wave is not None and input_img_spiral is not None:
    if st.button("Test"):
        ...
        # Combine confidences for each class
        confidences = {
            class_name1: [confidence_score1],
            class_name2: [confidence_score2],
            class_name3: [confidence_score3]
        }

        # Determine final decision
        final_decision = determine_final_score(confidences)
